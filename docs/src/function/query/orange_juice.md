...
- 参数：
  ...
  - `宠物机`： 可选，`标准`（抓宠物机）/ `夏日`（夏日抓宠物机）/ `赏金`（赏金猎人抓宠物机）/ `玩偶`（玩偶抓宠物机）
  - `次数`： 可选，`单抽` / `十连` / `百连`

- 详情：

  百橙抓宠物扭蛋，模拟在游戏里抓宠物，不填参数会展示宠物机选择
...
- 参数：
  ...
  - `日期`： 支持形如 `今天`/`明天`/`昨天`、`7.21`、`7月21日`、`2026-07-21`等格式

- 详情：

  查询指定日期混合器，过早/过晚的混合器可能不准确
