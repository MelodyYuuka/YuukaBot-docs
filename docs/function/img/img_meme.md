...
- 如果带前缀 `/` 或 [艾特](/about/glossary.html#%E8%89%BE%E7%89%B9)了机器人，则可无需完整参数，参数不完整时会提醒您缺失的参数，在后续补全参数即可
  - 例如 先发 `/5000生成`，然后继续发送 `5000兆円 欲しい！`
  - 支持补全的同时，一次性 `/5000生成 5000兆円 欲しい！` 这样也是没问题的！
  - `@机器人` + `指令` 也可以！例如：`@幽幽子 5000生成 5000兆円 欲しい！`
...
- 与指令一起发出
- 在发出指令后随后发出（需要指令前缀 `/` 或 [艾特](/about/glossary.html#%E8%89%BE%E7%89%B9)机器人）
- 通过回复使用之前消息作为参数
...
- 不加前缀 `/` 需要在发指令的时候一次性发送全部指令，若参数不完整不会返回任何信息
